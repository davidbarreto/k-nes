...
fn main() {
    ...
    let mut map: HashMap<String, assembler::types::Command> = HashMap::new();
    let command = assembler::parser::parse_line(" LABEL: JMP (LABEL)    ", 0x0000, &mut map);
    println!("Command: {:?}", command);
    println!("Symbol table: {:?}", map);
}
